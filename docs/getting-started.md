...
입니다. 따라서 먼저 필요한 것은 다음과 같습니다.

1. **Claude Code CLI**. 터미널에서 `claude --version` 이 동작해야 하고,
   한 번 실행해서 로그인이 끝나 있어야 합니다.
   - 설치 방법은 두 가지. [Claude Code 공식 안내](https://docs.claude.com/en/docs/claude-code/overview)
     의 절차를 따르세요.
     - **native installer** (Anthropic 이 제공하는 PowerShell·shell 설치
       스크립트). 단일 실행 파일로 깔리며 **Node.js 가 필요 없습니다**.
     - **npm** (`npm i -g @anthropic-ai/claude-code`). 이 경로를 쓰면
       Node.js 18+ 가 별도로 필요합니다.
   - hongtail 은 자식 프로세스를 그냥 `claude` 이름으로 spawn 하므로,
     어느 쪽으로 깔든 **PATH 에 `claude` 가 잡히기만 하면** 됩니다.
2. **Windows 10/11**. macOS·Linux 빌드 스크립트는 있지만 실제 동작 검증은
   Windows 에서만 이뤄졌습니다.

> hongtail 자체에는 Node.js 가 필요 없습니다 — 포터블 exe 안에 Electron
> 런타임이 다 들어 있습니다.
...
처음 켜면 selected 세션이 없으니 가운데가 비어 있는 빈 메인 화면이 뜹니다.

![hongtail 빈 메인 화면](screenshots/empty.png)

영역별로 어떤 일을 하는 곳인지부터 정리합니다.

| 위치 | 이름 | 역할 |
|---|---|---|
| 왼쪽 맨 위 | **Workspace 드롭다운 + 추가** | 작업할 폴더(=워크스페이스)를 추가·전환 |
| 왼쪽 위 | **1일 / 3일 / 7일 / 활성 / 모두** | 사이드바에 보일 세션 기간 필터 |
| 왼쪽 상단 | **+ 새 대화 / + 새 터미널** | 현재 워크스페이스에 세션을 추가 |
| 왼쪽 본문 | **세션 목록** | 현재 워크스페이스에 속한 세션들 (라이브 + 과거 jsonl) |
| 왼쪽 아래 | **⚙ 설정** | 폰트·언어·도구 카드 기본 펼침·웹 모드 등 |
| 가운데 | **메인 영역** | 세션을 고르면 메시지 영역·입력 박스·UsageBar 가 여기 채워짐 |
| 오른쪽 | **BTW 사이드 챗** | 메인 흐름을 막지 않고 옆에서 빠른 질문 |

세션을 고르고 한참 쓰면 가운데가 메시지·도구 카드로 가득 차고 하단에는
입력 박스와 UsageBar (모델·컨텍스트 사용량·권한 모드) 가 뜹니다.

![hongtail 활성 세션 모습](screenshots/main.png)
