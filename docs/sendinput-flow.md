...
hongtail 가 spawn 한 claude CLI 자식 프로세스에 어떻게 사용자 메시지를 전달하는지, stream-json input format 의 구조와 한계를 정리한 노트. 코드는 시간이 지나며 drift 하므로 **함수명·심볼**을 1차 anchor 로 잡고, 라인 번호는 보조 참고용.
...
- **string** — 단순 텍스트. 현재 hongtail 가 이것만 사용
...
`{type:"user", ...}` 외에 control event 도 있을 수 있지만 (예: interrupt) hongtail 는 현재 user message 만 보낸다.
...
`--replay-user-messages` 옵션을 켜면 자식이 user message 를 다시 emit 한다 (`stream-json` input/output 동시 사용 시에만). hongtail 는 이 플래그를 안 쓰고 있어 직접 echo 가 정답.
...
- main 의 `claude:send-input` 진입점에서 `text.startsWith('/')` 검사 → hongtail 가 자체 처리할 명령은 stdin 으로 보내지 않고 분기 (예: `/rename foo` → 세션/워크스페이스 alias 변경 IPC 로 라우팅)
...
> **History note**: 이전에는 stream-json input 에서 권한·모델 변경 같은 제어가 불가능하다고 결론 냈었고 (인터랙티브 TUI 전용), hongtail 는 spawn 옵션 `--permission-mode bypassPermissions` 로 우회만 했다. **claude CLI 2.1.119 부터** stream-json input 에 `control_request` 라는 별도 message 타입이 추가되어, 세션 도중 stdin 으로 제어가 가능하게 되었다. (출처: claude CLI 2.1.119 `cli.js` 코드 분석.)

권한 모드 변경 같은 제어용 메시지는 `claude:send-input` 이 아니라 **별도 IPC `claude:control-request`** 로 보낸다. main 은 다음 형식의 JSON 한 줄을 자식 stdin 에 write 한다:
...
자식이 처리하면 stdout 으로 `control_response` 가 흘러온다:

```json
// 성공
{"type":"control_response","response":{"subtype":"success","request_id":"..."}}

// 실패 (예: 환경상 bypassPermissions 사용 불가)
{"type":"control_response","response":{"subtype":"error","request_id":"...","error":"..."}}
```

renderer 의 `App.handleClaudeEvent` 에서 `extractControlResponse` 로 파싱해 `pendingControlRef` 에 등록된 요청과 매칭, 실패 시 이전 상태로 롤백한다 (`handleSetPermissionMode`).

### 지원되는 subtype (claude CLI 2.1.119 `cli.js` 기준)

| subtype | 의미 | request 추가 필드 |
|---|---|---|
| `set_permission_mode` | 도중에 권한 모드 변경 | `mode`: `plan` / `auto` / `acceptEdits` / `bypassPermissions` / `default` / `dontAsk` |
| `set_model` | 모델 변경 | `model`: `opus` / `sonnet` / `haiku` / `default` 또는 full id |
| `set_max_thinking_tokens` | thinking 예산 변경 | (예산 정수) |
| `interrupt` | 진행 중 turn 중단 | — |
| `can_use_tool` | tool permission gate 를 우리가 직접 처리할 때 응답하는 쪽 | (응답 형식 — gate 결정) |
| `initialize` | 세션 초기화 메시지. **자식이 시작 시 보내는** 방향 | — |
| `mcp_status` | MCP 서버 상태 질의 | — |

### 통신상 주의

- stdin 의 user message 와 control_request 는 같은 stream 을 공유한다. 자식 입장에서 둘은 **interleave 가능한 라인**.
- `request_id` 는 클라이언트가 발급 (uuid 등 고유 문자열). `control_response.response.request_id` 로 매칭해야 어느 요청에 대한 답인지 안다 — 여러 요청이 in-flight 일 수 있으므로 단순 FIFO 가정 금지.
- 실패 응답이 와도 stream 자체는 살아 있다. 권한 변경 실패 시 UI 상태만 롤백하고 세션은 계속.
- claude CLI 2.1.119 미만이거나 일부 환경에서는 일부 subtype 이 거부될 수 있다 (`bypassPermissions` 가 disabled 인 환경 등). UI 는 항상 응답을 기다리고 실패에 대비.

### 의의

- `/permissions`, `/model` 같은 인터랙티브 전용 슬래시 커맨드를 **stream-json 환경에서 대체** 할 수 있는 공식 채널이다.
- `interrupt` 는 사용자가 "stop" 버튼으로 진행 중인 turn 을 끊는 데 그대로 쓸 수 있다 (현재 hongtail 의 stop 은 child kill 인데, 그러면 재개 불가).
- `can_use_tool` 으로 tool permission gating 을 spawn 옵션 (bypassPermissions) 우회 대신 **per-tool 동의** 흐름으로 옮길 수 있다 — 향후 권한 UX 개선 여지.

## 핵심 인사이트

- hongtail 와 claude CLI 사이 채널은 **JSON Lines on stdin/stdout** 단 하나. user message, control_request 둘 다 같은 stdin 에 라인 단위로 흘린다. 응답도 stdout 한 stream 에 type 별로 섞여 들어온다.
