...
> **History note**: 이전에는 stream-json input 에서 권한·모델 변경 같은 제어가 불가능하다고 결론 냈었고 (인터랙티브 TUI 전용), hongluade 는 spawn 옵션 `--permission-mode bypassPermissions` 로 우회만 했다. **claude CLI 2.1.119 부터** stream-json input 에 `control_request` 라는 별도 message 타입이 추가되어, 세션 도중 stdin 으로 제어가 가능하게 되었다. (출처: claude CLI 2.1.119 `cli.js` 코드 분석.)
...
- `interrupt` 는 사용자가 "stop" 버튼으로 진행 중인 turn 을 끊는 데 그대로 쓸 수 있다 (현재 hongluade 의 stop 은 child kill 인데, 그러면 재개 불가).
