...
3. tail/watch 인프라가 hongtail 에 이미 있는가?
...
같은 sessionId 의 jsonl 안에 `entrypoint: "cli"` 와 `"sdk-cli"` 가 **섞일 수 있음**. 의미: hongtail `-p` 모드와 인터랙티브 모드가 같은 세션을 번갈아 열어도 둘 다 같은 jsonl 에 record 를 추가한다 (충돌 없음, append-only).
...
1. `src/main/session.ts` 에 `spawnInteractive(cwd, sessionId, mode)` — node-pty 로 `claude --session-id <uuid>` (또는 `--resume`) 띄움. 출력은 ANSI 그대로 버림 (보지 않음). hongtail 는 jsonl 만 본다.
