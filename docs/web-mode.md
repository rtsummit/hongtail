# Web 모드 — 외부 브라우저에서 hongtail UI 띄우기

Last Updated: 2026-04-27

`HONGTAIL_WEB=1` 환경변수로 활성화. main process 가 BrowserWindow 외에 별도
...
**비밀번호 설정**: 첫 활성화 시 비밀번호 미설정 — 설정 → 웹 모드 섹션에서
8자 이상 비밀번호를 set 해야 로그인 가능. 미설정 상태에서 `/login` 접근 시
"설정 필요" 안내 페이지가 뜬다. 사용자명 없음 — 비밀번호만으로 로그인.

credentials 는 `app.getPath('userData')/web-credentials.json` (Windows:
`%APPDATA%/hongtail/web-credentials.json`) 에 salt + sha256 hash 로 저장
(평문 저장 X). 비밀번호 변경 시 모든 기존 세션 무효화.

세션 cookie (`hongtail_s`):
...
브라우저                main process
                        ...
                        │
                        ├─ web.ts HTTP server (HONGTAIL_WEB=1 시)
...
- HTTPS 가 아니면 비밀번호 / 세션 cookie 가 평문 채널에 흐름. LAN 안에서도
  ...
  - 자체 종단: `HONGTAIL_WEB_TLS_CERT` + `HONGTAIL_WEB_TLS_KEY` 에 PEM 경로
...
- 세션 격리 없음 — 모든 클라이언트가 같은 view 를 본다. 이는 multi-device
  (나의 PC + 나의 폰) 가 같은 hongtail 인스턴스에 동시 붙는 의도된 동작.
...
MSYS_NO_PATHCONV=1 openssl req -x509 -newkey rsa:2048 -nodes \
  ...
  -subj "/CN=hongtail" \
  -addext "subjectAltName=DNS:localhost,DNS:hongtail.local,IP:127.0.0.1,IP:::1,IP:192.168.0.14"
```
...
### 1. tailscale (권장)
zero-trust VPN. hongtail 호스트 PC + 모바일에 tailscale 설치 → 호스트에
`HONGTAIL_WEB_HOST=127.0.0.1` 그대로 두고 tailscale 의 100.x.x.x 주소로
