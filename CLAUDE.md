...
- 자식 spawn 시 `--permission-mode <설정값>` + `--permission-prompt-tool stdio`. 모드는 `AppSettings.defaultPermissionMode` (기본 `bypassPermissions`, SettingsModal 에서 변경) 가 매 spawn 에 적용되며, 도중 변경은 UsageBar 의 mode 메뉴 / Shift+Tab 으로 control_request. 후자 플래그는 interactive deferred tool (AskUserQuestion / ExitPlanMode) 의 권한 요청을 자식 stdout 의 `can_use_tool` control_request 로 emit 시키는 핵심 (`docs/host-confirm-ui-plan.md` §11.1). 호스트는 카드 UI (`AskUserQuestionCard` / `ExitPlanModeCard`) 띄우고 사용자 응답을 stdin 으로 control_response 회신. 그 외 deferred tool 은 자동 allow fallback.
